...
class FrontDesk(http.Controller):
    ...
    @http.route('/front_desk/send_notification', type='json', auth='public')
    def send_notification(self, message):
        try:
            # Tìm hoặc tạo kênh trò chuyện
            channel = http.request.env['discuss.channel'].sudo().search([('name', '=', 'Notify')], limit=1)
            if not channel:
                channel = http.request.env['discuss.channel'].sudo().create({
                    'name': 'Notify',
                    'public': True,
                })

            # Đăng thông báo vào kênh trò chuyện
            channel.message_post(
                body=message,
                message_type='comment',  # Loại thông báo: comment
            ...
            http.request.env['mail.message'].sudo().create({
                'body': message,
                'message_type': 'notification', # message_type: notification, comment, email, newsletter
                'subtype_id': http.request.env.ref('mail.mt_comment').id,
                'model': 'discuss.channel',
                'res_id': channel.id,  # Liên kết thông báo với kênh trò chuyện
            })

            # Gửi thông báo đến tất cả người dùng
            for user in http.request.env['res.users'].sudo().search([]):
